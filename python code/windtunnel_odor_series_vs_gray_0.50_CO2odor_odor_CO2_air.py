...
try:
  ...
  for i in range(len(black_RGV_gray)):
    #Add timestamp for stimulus
    add_timestamp("start_" + "CO2 and odor_" + black_RGV_gray[i] )
    #Upload stimulus
    arduino_upload(black_RGV_gray[i])
    #Leave LED stimulus on for the StimTime
    time.sleep(StimTime-uploadTime)
    #Add timestamp for stimulus
    add_timestamp("end_" + "CO2 and odor_" + black_RGV_gray[i])
...
#--------------------------------------------------------------------------------
  ...
  for i in range(len(black_RGV_gray)):
    #Add timestamp for stimulus
    add_timestamp("start_" + "odor_" + black_RGV_gray[i] )
    #Upload stimulus
    arduino_upload(black_RGV_gray[i])
    #Leave LED stimulus on for the StimTime
    time.sleep(StimTime-uploadTime)
    #Add timestamp for stimulus
    add_timestamp("end_" + "odor_" + black_RGV_gray[i])
    #outputs time stamp to console
    print ("-- Stimulus period complete at %s --"%time_stamps[-1][1])
    #LEDs controller set to neutral state
    arduino_upload(arduino_neutral)
  ...
  #Restart the flow of CO2, and stops the flow of odor
  change_setpoint(CleanAirCtr, 168.5)
  change_setpoint(CO2_Ctr, 21.5)
  change_setpoint(OdorAirCtr, 0)
  ...
  for i in range(len(black_RGV_gray)):
    #Add timestamp for stimulus
    add_timestamp("start_" + "CO2_" + black_RGV_gray[i] )
    #Upload stimulus
    arduino_upload(black_RGV_gray[i])
    #Leave LED stimulus on for the StimTime
    time.sleep(StimTime-uploadTime)
    #Add timestamp for stimulus
    add_timestamp("end_" + "CO2_" + black_RGV_gray[i])
    #outputs time stamp to console
    print ("-- Stimulus period complete at %s --"%time_stamps[-1][1])
    #LEDs controller set to neutral state
    arduino_upload(arduino_neutral)
  ...
  #Stops the flow of odor and CO2. Raise the flow of clean air to compensate
  change_setpoint(CleanAirCtr, 190)
  change_setpoint(CO2_Ctr, 0)
  change_setpoint(OdorAirCtr, 0)
  ...
  for i in range(len(black_RGV_gray)):
    #Add timestamp for stimulus
    add_timestamp("start_" + "clean air_" + black_RGV_gray[i] )
    #Upload stimulus
    arduino_upload(black_RGV_gray[i])
    #Leave LED stimulus on for the StimTime
    time.sleep(StimTime-uploadTime)
    #Add timestamp for stimulus
    add_timestamp("end_" + "clean air_" + black_RGV_gray[i])
    #outputs time stamp to console
    print ("-- Stimulus period complete at %s --"%time_stamps[-1][1])
    #LEDs controller set to neutral state
    arduino_upload(arduino_neutral)
