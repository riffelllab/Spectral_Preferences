...
totalExpDuration = \
  ...
  len(upsweep) * (StimTime + interStimTime) + \
...
try:
  ...
  for i in range(len(upsweep)):
    #Add timestamp for start of stimulus
    add_timestamp("start_" + "spectral upsweep_" + upsweep[i] )
    #Upload stimulus
    arduino_upload(upsweep[i])
    #Leave LED stimulus on for the StimTime
    time.sleep(StimTime-uploadTime)
    #Add timestamp for end of stimulus
    add_timestamp("end_" + "spectral upsweep_" + upsweep[i])
    #outputs time stamp to console
    print ("-- Stimulus period complete at %s --"%time_stamps[-1][1])
    #LEDs controller set to neutral state
    arduino_upload(arduino_neutral)
