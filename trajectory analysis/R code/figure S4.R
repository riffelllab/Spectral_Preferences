...
recruit.model <- 
  glmmTMB(resp ~ 0 + odor + treatment_factor:odor + (1|rep), 
...
recruit.model.summary <- 
  #extracts coefficients
  summary(recruit.model)$coefficients$cond %>% 
  as.data.frame() %>% 
...
#Full Model
recruit.model.1 <- glmmTMB(resp ~ trt_color + trt_intensity + odor + 
...
recruit.model.2 <- glmmTMB(resp ~ trt_color + trt_intensity + odor + 
                          trt_color:trt_intensity + trt_color:odor + trt_intensity:odor + 
                          (1|rep), 
                        data = events.sub, 
                        family = betabinomial(link = "logit"))
...
#no odor model
recruit.model.3 <- glmmTMB(resp ~ trt_color + trt_intensity + 
...
#no color model
recruit.model.4 <- glmmTMB(resp ~ trt_intensity + odor + 
...
#no intensity model
recruit.model.5 <- glmmTMB(resp ~ trt_color + odor + 
                          trt_color:odor + (1|rep), 
                        data = events.sub, 
                        family = betabinomial(link = "logit"))
summary(recruit.model.5)

#anova testing the effect of intensity
anova(recruit.model.1, recruit.model.5)
...
model.pred<-
  ...
  left_join(recruit.model.summary %>% select(odor, treatment_factor, "Pr(>|z|)"), 
...
#drop treatment_factor:odor combinations with no data
activ.model <- 
  glmmTMB(n_tracks ~ odor + treatment_factor:odor + (1|rep), 
...
resp_check_odor <- as.numeric(trt_lvls$treatment_factor == "resp check, initial odor" )
...
contrasts<-
  contrast(activ.model.emm, adjust = "sidak", method = 
             list("resp_check_init - PreCO2Time"     = resp_check_init - PreCO2Time,
                  ...
                  "resp_check_odor - PreStim"        = resp_check_odor - PreStim,
...
yaxis_ticks_large<-seq(0, ylim_large[2], 0.05)
yaxis_ticks_small<-seq(0, ylim_small[2], 0.05)
...
subplot_a_height <- ylim_large[2]/ylim_small[2] * subplot_height + m_top + m_bottom
subplot_b_height <- ylim_large[2]/ylim_small[2] * subplot_height + m_top + m_bottom
subplot_c_height <- subplot_height + m_top + m_bottom
subplot_d_height <- subplot_height + m_top + m_bottom
subplot_e_height <- subplot_height + m_top + m_bottom
...
od_label.y_large <- 0.162
od_label.y_small <- 0.082
od_label.cex <-1
...
axis(side=2, las=1, at=yaxis_ticks_large, mgp=axis_mgp, tck=tck1, cex.axis=cex.axis)
...
mtext("proportion recruited", side=2, line=2.2)
...
labels <-
  ...
  pull(n_tracks_resp) %>%
  paste0("(", ., ")" )
text(x=display.info.CO2$x.pos, y=rep(sample_size_ypos_large,length.out=n_trts),
...
text(x=od_label.x, y=od_label.y_large, labels=bquote(bold(CO[2]~"alone")),
...
labels <-
  ...
  pull(n_tracks_resp) %>%
  paste0("(", ., ")" )
text(x=display.info.CO2_VGrY$x.pos, y=rep(sample_size_ypos_large,length.out=n_trts),
     adj=c(0.5,1), labels=labels, cex=sample_size_cex, col=sample_size_col)
...
lines(x=xaxis_pos[7:8], y=rep(ylim_large[1],2))
lines(x=xaxis_pos[9:10], y=rep(ylim_large[1],2))
axis(side=2, las=1, at=yaxis_ticks_small, mgp=axis_mgp, tck=tck2, cex.axis=cex.axis)
...
labels <-
  ...
  pull(n_tracks_resp) %>%
  paste0("(", ., ")" )
text(x=display.info.tansy$x.pos, y=rep(sample_size_ypos_small,length.out=n_trts),
     adj=c(0.5,1), labels=labels, cex=sample_size_cex, col=sample_size_col)
...
text(x=od_label.x, y=od_label.y_small, labels=bquote(bold("floral odor")),
     adj=c(0.5,0.5), cex=od_label.cex)
...
labels <-
  ...
  pull(n_tracks_resp) %>%
  paste0("(", ., ")" )
text(x=display.info.foot$x.pos, y=rep(sample_size_ypos_small,length.out=n_trts),
     adj=c(0.5,1), labels=labels, cex=sample_size_cex, col=sample_size_col)
...
text(x=od_label.x, y=od_label.y_small, labels=bquote(bold("host odor")),
     adj=c(0.5,0.5), cex=od_label.cex)
...
labels <-
  ...
  pull(n_tracks_resp) %>%
  paste0("(", ., ")" )
text(x=display.info.alfalfa$x.pos, y=rep(sample_size_ypos_small,length.out=n_trts),
     adj=c(0.5,1), labels=labels, cex=sample_size_cex, col=sample_size_col)
...
text(x=od_label.x, y=od_label.y_small, labels=bquote(bold("plant infusion")),
     adj=c(0.5,0.5), cex=od_label.cex)
