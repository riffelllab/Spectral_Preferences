...
#output_dir<-"output files/color vs gray 0.50, air, CO2, foot, CO2 + foot/"
output_dir<-"../figures/"
...
sub_dir<-"output files/color vs gray 0.50, air, CO2, foot, CO2 + foot/"
...
pref.model.emm<-emmeans(pref.model, specs = ~ trt_color:CO2:odor)
...
contrast(pref.model.emm, adjust = "none", method = 
           list("CO2Yes_odorNo - CO2Yes_odorYes" = CO2Yes_odorNo - CO2Yes_odorYes))


#---- Analysis with only CO2 runs
tracks.sub<-
  tracks %>%
...
#Run Anova for effect of odor
anova(pref.sub.model, pref.sub.model.no.odor)

#Run Anova for overall model fit
anova(pref.sub.model, pref.null)
...
#no CO2 model
activ.model.1 <- glmmTMB(n_tracks ~ 0 + trt_color:odor + (1|rep),
                         data = events, family = nbinom1(link = "log"))
...
#no odor model
activ.model.2 <- glmmTMB(n_tracks ~ 0 + trt_color:CO2 + (1|rep),
                         data = events, family = nbinom1(link = "log"))
summary(activ.model.2)

#likelhood ratio test to test significance of CO2 effect
anova(activ.model, activ.model.2)
...
#uses dummy variable to contrast all other treatment levels with no CO2, no odor, grey
activ.model.sig <- glmmTMB(resp ~ dummy + (1|rep),
                             data = events, family = betabinomial(link = "logit"))
...
yaxis_ticks.sp1<-seq(ylim.sp1[1], ylim.sp1[2], 100)
...
sig_star_cex <- 9.5/8 #size of significance stars
...
lines( x = c( rep(display.info.pref$x.pos[8], 2),
              rep(display.info.pref$x.pos[18], 2) ),
       y = c( -0.5*sig_star_spacer.sp2, 0, 0, -0.5*sig_star_spacer.sp2) + bar.3.y, 
       lwd = sb.lwd )
text( x = mean( display.info.pref$x.pos[c(8,18)] ),
      y = bar.3.y  + 0.5*sig_star_spacer.sp2,
      labels = "*", adj = c(0.5,0.5), cex=sig_star_cex )
...
lines( x = c( rep(display.info.pref$x.pos[8], 2),
              rep(display.info.pref$x.pos[18], 2) ),
       y = c( -sig_star_spacer.sp3, 0, 0, -sig_star_spacer.sp3) + bar.1.y, 
       lwd = sb.lwd )
text( x = mean( display.info.pref$x.pos[c(8,18)] ),
      y = bar.1.y  + 2*sig_star_spacer.sp3,
      labels = "n.s.", adj = c(0.5,0.5) )
